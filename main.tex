...
$$\int_{(n-1)T}^{nT}g_i(t)dt\simeqTg_i[(n-1)T]$$
$$\therefore x_i(nT)-x_i[(n-1)T]=Tg_i[(n-1)T]$$
$$x_i(\tau)-z^{-1}x_i(z)=Tz^{-1}G_i(z)$$
$$({z-1\over T})x_i(z)=G_i(z)\Rightarrow\tcolorbox{S_a={z-1\over T}\quad\text{Substitute this expression in transform}}$$
...
\tcolorbox{let $s_a={1-z^{-1}\over T}={z-1\over ZT}$ do transform}

\begin{enumerate}
\item Rational? O.K.
\item MApping into unit circle? $Z={1\over 1-jw_aT}={1+jw_aT\over 1+w_a^2T^2}$\\
$|z|=1\text{ only for }w_a=0 (w_a\ll 1/T)$
\item Stability? Poles move away from unit circle in Z-plane as poles move towards $jw_a$ axis in s-plane. Not satisfied.
\end{enumerate}
